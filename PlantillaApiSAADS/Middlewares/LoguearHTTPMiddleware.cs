...
using PlantillaApiSAADS.ModelsLog;
using System;
...
namespace PlantillaApiSAADS.Middlewares
{
    ...
    public class LoguearHTTPMiddleware
    {
        ...
        public LoguearHTTPMiddleware(RequestDelegate siguiente, IServiceProvider serviceProvider)
        {
        ...
        private void AddLog(LogApiSaad log, UpdsLogContext contextdb)
        {
            // toodos metodos menos el GET
            if (!log.Path.Contains("swagger") && log.Method != "GET")
            {
                _ = Task.Run(async () => {
                    try
                    {
                        ...
                        {
                            var contextdb = scope.ServiceProvider.GetRequiredService<UpdsLogContext>();
                            contextdb.Add(log);
